# -*- coding: utf-8 -*-
"""
Created on Sun Apr 12 13:27:26 2020
...
def optimize(X,y,delta, alpha):
    ...
    D = np.zeros((2*n+m+2))
    D[2*n+m+2-1] = K-1   #s2 class 0
    D[2*n+m+2-2] = M-1   #s1 class 1
    for i in range (n):
        C[i,i] = 0#1
        C[i+n,i] = 0#-1
        C[i, i+n+m] = 0#-1
    ...
    # print("opt_A:"+str(A))     
    # print("opt_B:"+str(B))       
    res = linprog(W, A_ub=A, b_ub=B,bounds=(None, None),method = 'simplex') 
    wd = res.x[:n]
    qi = np.zeros((m))
    for i in range(m):
        qi[i] = res.x[i+n]
...
def optimize_A(X, y, C1, C3, C4, delta, alpha):
    print("opt_A")
    print("C1:"+str(C1))
    print("C3:"+str(C3))
    print("C4:"+str(C4))
    ...
    print("********")
    (l1, l2) = X.shape
    X1 = np.zeros((len(C3)+len(C4), l2))
    y1 = np.zeros((len(C3)+len(C4)))
    X_C1 = np.zeros((len(C1), l2))
    y_C1 = np.zeros((len(C1)))
    j = 0
    k = 0
    for i in range(l1):
        if i in C1:
    ...
    C = np.zeros((2*n, n+m+n))
    D = np.zeros((2*n+m+len(C1)+1))
    # D[2*n+2-1] = -1
    # D[2*n+2-2] = -1
    D[2*n+m :2*n+m+len(C1)] = -delta
    D[2*n+m+len(C1)] = M-1
    #####################2n################
    for i in range (n):
        C[i,i] = 0#1
    ...
    ###############len(C1)#################
    C_C1 = X_C1
    C_C1 = np.hstack((C_C1, np.zeros((len(C1), m + n))))
    C = np.vstack((C, C_C1))
    ######################################
    #################1####################
    C = np.vstack((C, s1)) #class 1
    # C = np.vstack((C, s2)) #class 0
    ######################################
    print("X_C1")
    print(X_C1)     
    A = np.vstack((A,C))
...
def optimize_B(X, y, C2, C3, C4, delta, alpha):
    print("opt_B")
    print("C2:"+str(C2))
    print("C3:"+str(C3))
    print("C4:"+str(C4))
    ...
    X1 = np.zeros((len(C3)+len(C4), l2))
    y1 = np.zeros((len(C3)+len(C4)))
    X_C2 = np.zeros((len(C2), l2))
    y_C2 = np.zeros((len(C2)))
    j = 0
    k = 0
    for i in range(l1):
        if i in C2:
    ...
    #Ax=B the equality contraints
    C = np.zeros((2*n, n+m+n))
    D = np.zeros((2*n+m+len(C2)+1))
    # D[2*n+2-1] = -1
    # D[2*n+2-2] = -1
    D[2*n+m :2*n+m+len(C2)] = -delta
    D[2*n+m+len(C2)] = M-1
    ###################2n##################
    for i in range (n):
        C[i,i] = 0#1
    ...
    #############len(C2)##################
    C_C2 = X_C2
    C_C2 = np.hstack((C_C2, np.zeros((len(C2), m + n))))
    C = np.vstack((C, C_C2))
    ######################################
    ####################1#################
    C = np.vstack((C, s1)) #class 1
...
#         print(y_res>-1*delta)
        for i in range(n):
            if y_res[0][i] >= delta - 0.01:
                y_res[0][i] = 1
            elif y_res[0][i] <= -delta + 0.01:
                y_res[0][i] = 0
            else :
                y_res[0][i] = 1-y[i]
    else:
...
def quantize(a,b,twin_node,XX):
    if(twin_node == 0):
        ...
        for j in range(res.shape[0]):
            if(res[j][0] >= delta-0.01):
                res[j][0] = 1
            elif(res[j][0] <= -delta+0.01):
                res[j][0] = 0
            else:
...
#         print(Wa)    
    ...
    C3 = C3_new
    C4 = C4_new    
    W_A,clf = optimize_A(X_A, y_A, C1, C3, C4, delta, alpha)
...
#     print(X_B)
    ...
    W_B,clf = optimize_B(X_B, y_B, C2, C3, C4, delta, alpha)
